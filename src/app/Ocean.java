...
package, so look that up in the Java API. */
		...
		while(!battleship.okToPlaceShipAt(row, col, horizontal, this));
...
from the BattleshipGame class. */
		...
		for(int row=0; row<ships.length; row++) {
			...
			for(int col=0; col<ships[row].length; col++) {
				 // BUT HOW DO I KNOW IF I'VE HIT ANY OF THESE LOCATIONS...?
				// --> Just print the object, I am setting them all to print the correct character.
